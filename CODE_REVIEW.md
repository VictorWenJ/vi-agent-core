...
| **Lombok 与工具类** | 日志类是否优先使用 `@Slf4j`，依赖注入类是否优先使用 `@RequiredArgsConstructor`？ | 允许使用 Lombok 简化样板代码，但不得因使用注解而弱化语义边界。 |
| | 是否在 DTO / VO / 简单状态对象上合理使用 `@Data` / `@Builder` / 构造器注解？ | 可简化样板代码；但领域核心模型、需精细控制语义的方法对象应谨慎使用 `@Data`。 |
| | 是否重复实现了已有公共工具逻辑（如 JSON 转换）而未复用 `JsonUtils`？ | 已有公共工具应优先复用；新增通用逻辑应统一沉淀到 `common/util`。 |
| | 是否把业务编排或有状态逻辑塞进工具类？ | 工具类只能承载无状态通用能力，不能承担 Runtime 编排职责。 |
