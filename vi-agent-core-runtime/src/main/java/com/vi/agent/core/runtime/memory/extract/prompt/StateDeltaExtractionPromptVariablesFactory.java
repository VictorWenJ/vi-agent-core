 ...
 */
@Component
public class StateDeltaExtractionPromptVariablesFactory {

    /**
     * 将状态增量抽取命令转换为 prompt renderer 变量。
     */
    public Map<String, String> variables(StateDeltaExtractionCommand command) {
        ...
        variables.put("currentStateJson", command == null || command.getCurrentState() == null ? "{}" : JsonUtils.toJson(command.getCurrentState()));
        variables.put("conversationSummaryText", command == null || command.getConversationSummary() == null
            ? ""
            : StringUtils.defaultString(command.getConversationSummary().getSummaryText()));
     ...
     */
    private String renderTurnMessages(StateDeltaExtractionCommand command) {
