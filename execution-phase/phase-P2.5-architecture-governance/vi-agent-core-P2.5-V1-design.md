...
7. Checkpoint / Interrupt / Replay、Artifact、Streaming、Model Routing、Tool Sandbox、Approval、Redaction、Side-effect Idempotency 等一等边界，避免后续 Graph / Tool / Agent 能力落地时重新打散架构。
...
6. 对象进入 `contract` 前必须记录“使用模块清单 + 为什么不能归属某个能力模块”。
...
RuntimeResumeToken / ArtifactRefContract 等轻量引用协议
基础枚举
```
...
完整 Artifact / Workspace / Checkpoint / Approval 领域对象
```
...
每个 contract 对象都必须能回答：哪些模块使用它、为什么不能归属到某个能力模块、为什么不能通过目标模块 api/port 暴露。
```
...
8. 密钥、数据库密码、Redis 密码、外部 API Key 在项目开发阶段在暂时可以放入模块 resources。
9. 现阶段采用模块化单体，不将 prompt、tool、skill、memory 等模块拆成独立 Docker 服务。
...
| 缺少 checkpoint / interrupt / replay 一等边界 | 在 Graph 边界定义 `GraphCheckpointStore`、`RuntimeResumeToken`、`InterruptRequest`、`ResumeCommand`、`CheckpointNamespace` |
| Memory 与 checkpoint / audit / artifact 边界不清 | 明确 Checkpoint、SessionMemory、LongTermMemory、Artifact、Audit 五类数据边界 |
| Tool 缺少 MCP / sandbox / approval 执行边界 | 增加 ToolTransport、ToolExecutionSandbox、ToolApprovalRequest、ToolTimeoutPolicy、CancellationToken、ToolResultArtifactRef |
| 缺少 Artifact / Workspace 产物层 | 新增 `vi-agent-core-artifact` 作为产物生命周期模块 |
| Streaming / model routing / redaction / side-effect 幂等不足 | 增加统一事件模型、模型路由策略、脱敏策略、side-effect idempotency 边界 |

---

### 2.9 Checkpoint / Memory / Artifact / Audit 边界分离

P2.5 必须明确执行恢复、记忆、产物、审计的边界，避免把所有运行数据都塞进 memory 或 audit。

| 边界 | 归属 | 用途 | 不负责 |
|---|---|---|---|
| Checkpoint | `graph` 边界 + `storage` 实现 | 执行恢复、interrupt/resume、replay、fault tolerance | 不承载长期记忆，不作为用户知识召回源 |
| SessionMemory | `memory/session` | 当前 session / conversation 上下文闭环 | 不保存完整执行恢复状态 |
| LongTermMemory | `memory/longterm` | 跨 session 可召回的偏好、事实、目标、项目记忆 | 不保存临时 graph step 状态 |
| Artifact | `artifact` | 文件、报告、patch、截图、导出物、任务产物生命周期 | 不等同于 memory，不承担审计事实链 |
| Audit | `audit` | 事实追踪、链路还原、合规查询 | 不作为恢复状态，不作为模型长期记忆 |
...
| `vi-agent-core-artifact` | 任务产物、文件、报告、patch、截图、导出物的生命周期与引用管理 |
...
#### 4.4.6 Streaming 统一事件模型

`lifecycle` 负责定义对外 streaming 事件顺序和取消语义。Graph、Tool、Agent、Approval 等内部事件不得直接暴露为随机格式，必须转换为统一 `RuntimeStreamEvent`。
...
|---|---|
| `RUN_STARTED` | run 开始 |
| `RUNTIME_STEP_STARTED` / `RUNTIME_STEP_FINISHED` | 固定 lifecycle step 开始/结束 |
| `GRAPH_NODE_STARTED` / `GRAPH_NODE_FINISHED` | Graph node 开始/结束 |
| `AGENT_HANDOFF` | Main/SubAgent 委派或 handoff |
| `MODEL_TOKEN_DELTA` | 模型 token 增量 |
| `TOOL_STARTED` / `TOOL_FINISHED` | 工具执行开始/结束 |
| `APPROVAL_PENDING` | 需要人工确认，携带 resume token |
| `ARTIFACT_CREATED` | 产物生成或更新 |
| `ERROR` | 可见错误事件 |
| `DONE` | 流式响应完成 |
...
```text
RUN_STARTED 必须先于其他事件。
DONE 是成功流的最后事件。
ERROR 是失败流的终止事件，除非被标记为 DEGRADED。
APPROVAL_PENDING 会中断当前流，等待 ResumeCommand 后继续。
取消请求必须传播到 GraphKernel / ToolRuntime / ProviderGateway，并生成 cancellation audit。
```
...
#### 4.5.3 CapabilitySnapshot 事实源

`CapabilitySnapshot` 的事实源是 `AgentCapabilityResolver` / `CapabilityPackageAssembler` 在 `AGENT_EXECUTION` 开始前生成的能力解析结果。

| 对象 | 归属 | 说明 |
|---|---|---|
| `EffectiveAgentCapabilities` | agent / capability 解析边界 | 内存态有效能力集合 |
| `CapabilitySnapshot` | audit 记录 + storage 持久化实现 | 本次执行实际可见能力快照 |
| `GraphState.capabilitySnapshotId` | graph | Graph 执行引用该快照，不复制完整内容 |
| `GraphCheckpoint.capabilitySnapshotId` | checkpoint 边界 | resume/replay 时复用或校验能力快照 |
| `Memory` | 不拥有 CapabilitySnapshot | memory 不存储能力快照，只可引用 evidence/artifact/memory refs |

规则：

```text
CapabilitySnapshot 是审计和复现事实，不是长期记忆。
CapabilitySnapshot 可以被 GraphCheckpoint 引用，用于恢复时校验执行能力是否发生漂移。
CapabilitySnapshot 不应被写入 SessionMemory / LongTermMemory。
```

#### 4.5.4 AGENT_EXECUTION 内部执行来源
...
#### 4.6.4 Checkpoint / Interrupt / Replay 一等边界

Graph Kernel 的核心不仅是节点列表，还必须具备执行恢复边界。P2.5 不实现完整 checkpoint，但必须冻结接口归属和数据边界。

| 概念 / 模型 | 说明 |
|---|---|
| `GraphCheckpointStore` | Graph checkpoint port，持久化由 storage 实现 |
| `GraphCheckpoint` | 某个 graph run 在 super-step / node 边界的状态快照 |
| `CheckpointNamespace` | checkpoint 命名空间，用于隔离 run、graph、agent、tenant、workspace |
| `RuntimeResumeToken` | 对外恢复令牌，返回给客户端或上层 runtime |
| `InterruptRequest` | 中断请求，如等待人工确认、外部输入、长任务暂停 |
| `ResumeCommand` | 恢复命令，携带 resume token、审批结果或外部输入 |
| `ReplayCommand` | 重放命令，用于调试、回归和 time travel 预留 |

边界规则：

```text
Checkpoint 保存执行恢复状态，不保存长期记忆。
InterruptRequest 由 GraphKernel 产生，lifecycle 负责转成 APPROVAL_PENDING / INTERRUPTED 响应事件。
ResumeCommand 先由 lifecycle 校验，再交回 GraphExecutionFacade 恢复。
GraphCheckpointStore 只保存 GraphState / node cursor / capabilitySnapshotId / pending action refs 等恢复必要数据。
Audit 记录发生过什么；Checkpoint 记录如何恢复。两者不能替代。
```

ASK / Human Approval 与 checkpoint 的关系：

```text
Tool / GraphRoutePolicy 返回 ASK
  -> GraphKernel 生成 InterruptRequest
  -> GraphCheckpointStore 保存 pending checkpoint
  -> lifecycle 返回 RuntimeResumeToken + APPROVAL_PENDING event
  -> 用户审批后提交 ResumeCommand
  -> GraphKernel 从 checkpoint 恢复并继续执行
  -> Audit 记录 approval request/result/resume
```
...
| `ToolTransport` | 工具调用传输方式，如 local function、MCP、HTTP、sandbox executor |
| `ToolExecutionSandbox` | 工具执行沙箱边界，用于 shell、browser、code runner、document converter 等高风险执行 |
| `ToolTimeoutPolicy` | 工具超时策略 |
| `CancellationToken` | 工具取消令牌，用于流式取消、超时取消和用户取消 |
| `ToolApprovalRequest` | 工具执行前的审批请求，由 Safety ASK 触发 |
| `ToolResultArtifactRef` | 工具执行产生的文件、报告、截图、patch 等产物引用 |
| `ToolOutputRedactionPolicy` | 工具输出脱敏策略 |
| `ToolOutputTrustMark` | 工具输出信任标记，用于提示词注入防护 |
...
#### 4.8.2 Tool Transport / MCP / Sandbox 边界

Tool 执行不等于本地 Java 方法调用。P2.5 应将执行通道抽象出来，为未来 MCP、HTTP、sandbox、browser/computer/code runner 预留边界。

| ToolTransport | 说明 |
|---|---|
| `LOCAL_FUNCTION` | 本地函数或 Spring Bean 工具 |
| `MCP` | MCP Server 工具接入 |
| `HTTP` | 远程 HTTP 工具服务 |
| `SANDBOX` | 沙箱执行器，如 shell/code/browser/document converter |

规则：

```text
ToolRuntime 负责权限、审计、校验、超时、取消和结果归一化。
ToolExecutor 负责具体执行。
高风险 ToolExecutor 可以未来独立部署，但 ToolRuntime 控制面仍在主应用内。
MCP 作为 ToolTransport 的一种实现，不直接绕过 ToolPolicy / Safety / Audit。
```

#### 4.8.3 Side-effect Idempotency / Timeout / Cancellation

带副作用的工具必须具备幂等和取消边界。

| 概念 | 说明 |
|---|---|
| `ToolIdempotencyKey` | 一次工具副作用执行的幂等键 |
| `SideEffectType` | READ / WRITE / DELETE / EXECUTE / SUBMIT / PAYMENT 等 |
| `CompensationPolicy` | 失败或重试后的补偿策略预留 |
| `ToolTimeoutPolicy` | 超时边界 |
| `CancellationToken` | 用户取消、超时取消、Graph 中断时向下游传播的取消令牌 |

规则：

```text
READ 类工具可重试。
WRITE / DELETE / EXECUTE / SUBMIT 类工具重试前必须检查 idempotency key。
Graph retry 不能直接重复执行已产生副作用的工具。
ToolExecution 需要记录 idempotencyKey、sideEffectType、approvalId、artifactRefs 和 audit refs。
```

#### 4.8.4 Tool Output Redaction 与 Prompt Injection 标记

工具输出进入 prompt 前必须标记来源与信任级别。

规则：

```text
Tool output 默认视为 untrusted data。
进入 Prompt 时必须放入 data/tool_result block，不能作为 instruction。
Tool output 存入 audit 前必须应用 redaction policy。
包含 HTML、脚本、外部网页、用户上传文件内容的工具结果必须带 prompt-injection risk 标记。
```
...
#### 4.12.1 Model Routing / Fallback / Capability Negotiation

Provider 模块需要支撑模型路由和能力协商，但 P2.5 只冻结职责边界，不冻结完整策略字段。

| 概念 / 模型 | 说明 |
|---|---|
| `ModelProfile` | 模型画像，包含 provider、model、temperature、max tokens、cost level 等 |
| `ModelRoutingPolicy` | 根据 AgentProfile、Skill、任务类型、预算选择模型 |
| `ModelFallbackPolicy` | 模型失败、限流、能力不匹配时的 fallback 策略 |
| `RateLimitPolicy` | 请求速率限制策略 |
| `BudgetPolicy` | token / cost / latency 预算策略 |
| `ProviderCapabilityNegotiator` | 判断 provider 是否支持 structured output、tool calling、streaming、vision、embedding 等能力 |

规则：

```text
Agent / Prompt 不直接判断具体 provider 能力。
结构化输出、tool calling、streaming 是否可用由 ProviderCapabilityNegotiator 返回。
ModelRoutingPolicy 的决策结果应进入 capability snapshot 和 model call audit。
```
...
#### 4.14.3 Redaction 与可见性策略

Audit 不等于原始数据仓库。所有写入 audit、trace、prompt render record、tool arguments、memory write candidate 的内容必须经过脱敏策略。

| 数据位置 | 默认处理 |
|---|---|
| Prompt 输入 | 存 summary / hash / block refs，敏感 data block 脱敏 |
| Tool arguments | 高敏字段 mask，保留 schema 和摘要 |
| Tool output | 默认 untrusted，存摘要、artifact refs 和 redaction metadata |
| Model response | 可存摘要、usage、结构化结果；长文本按策略截断 |
| Memory candidate | 保存 evidence refs 和变更摘要，敏感值按 policy 处理 |
| Approval request | 保存风险原因、动作摘要、参数摘要，不保存明文 secret |

规则：

```text
Audit payloadJson 只能保存可审计必要信息。
敏感字段必须有 redactionLevel。
可展示内容和可存储内容必须分开。
raw prompt / raw tool result / raw file content 不得无策略全量写入 audit。
```

#### 4.14.4 动态 SubAgent 审计树
...
#### 4.15.5 ASK / Human Approval 执行闭环

`ASK` 不是单纯枚举，必须触发可恢复的 runtime 中断。

```text
Safety 返回 ASK
  -> GraphKernel 生成 InterruptRequest
  -> GraphCheckpointStore 保存 checkpoint + pending action ref
  -> Audit 记录 SafetyDecisionAudit / ApprovalRequestAudit
  -> lifecycle 返回 APPROVAL_PENDING 事件和 RuntimeResumeToken
  -> 用户提交审批结果
  -> lifecycle 接收 ResumeCommand
  -> GraphKernel 读取 checkpoint 并继续或终止
  -> Audit 记录 ApprovalResultAudit / ResumeAudit
```
...
graphRunId
graphNodeExecutionId
toolInvocationRef / memoryWriteCandidateRef / actionRef
...
```text
没有 checkpoint 的 ASK 不能进入生产主链路。
审批结果必须进入 audit。
审批通过后仍需校验 pending action 是否过期、是否被篡改、是否仍匹配原始 capability snapshot。
```

#### 4.15.6 Safety 与其他模块的边界
...
#### 4.16.1 Online Validation / Offline Eval / Evaluator Agent 边界

Evaluation 分为三类边界：

| 类型 | 说明 | 主链路依赖方式 |
|---|---|---|
| Online Validation | 输出契约、工具结果、证据、计划校验 | 可被 GraphNode / ToolRuntime / PromptRuntime 调用 |
| Offline Eval | Golden dataset、scenario、regression、RAG quality、cost/latency | 不应被在线主链路强依赖 |
| Evaluator Agent | 以 AgentProfile 形式存在的评估 Agent | 由 Graph 通过 AgentExecutorPort 调用，不由 evaluation 模块直接执行 workflow |
...
在线主链路只能依赖 online validation api。
离线评测系统不得成为 chat/runtime 的启动依赖。
Evaluator Agent 是 agent profile，不是 evaluation 模块里的特殊运行时。
```
...
### 4.18 `vi-agent-core-artifact`

`artifact` 负责任务产物生命周期。它与 memory、audit、checkpoint 分离。

| 概念 / 模型 | 说明 |
|---|---|
| `ArtifactRecord` | 产物记录，如文件、报告、patch、截图、导出物 |
| `ArtifactRef` | 跨模块轻量产物引用；轻量协议可进入 contract，完整记录归 artifact |
| `ArtifactType` | 产物类型，如 file、patch、report、screenshot、document、dataset |
| `ArtifactVersion` | 产物版本 |
| `ArtifactHash` | 产物内容 hash，用于去重、校验和审计 |
| `ArtifactOrigin` | 产物来源，如 tool、agent、rag、user upload、export |
| `ArtifactPermissionPolicy` | 产物访问权限策略 |
| `ArtifactLifecyclePolicy` | 产物保留、过期、删除、归档策略 |
| `WorkspaceRef` | 工作区引用，用于 coding/document/browser 等场景隔离产物 |
| `ArtifactRepository` | artifact 模块 port，storage 提供实现 |

边界规则：

```text
Artifact 保存任务产物和可下载/可引用对象。
Memory 可以引用 ArtifactRef，但不保存 artifact 内容。
Audit 记录 artifact 如何产生、由谁产生、何时产生。
Checkpoint 可以引用 ArtifactRef 用于恢复，但不保存 artifact 内容。
Storage 负责 artifact metadata / object location 的持久化实现。
```

---

### 4.19 `vi-agent-core-app`
...
artifact  -> contract, common, bootstrap, audit api, safety api
...
lifecycle -> contract, common, bootstrap, context api, memory api, artifact api, graph api, audit api
```
...
storage -> 各模块 port（包含 artifact port）
```
...
### 5.5 模块依赖矩阵与 Public API 规则

所有模块必须遵守可检查的 import 边界。后续应通过 Maven Enforcer / ArchUnit 固化。

| 被依赖包 | 是否允许跨模块 import | 说明 |
|---|---|---|
| `common.*` | 允许 | 通用工具和异常 |
| `contract.*` | 允许 | 最小公共协议 |
| `{module}.api.*` | 允许 | 模块对外 Facade / Command / Result / DTO |
| `{module}.port.*` | 允许特定方向 | storage / adapter 实现 port，业务模块不反向依赖 adapter |
| `{module}.config.*` | 默认不允许 | 只由 app 或模块自身启动使用 |
| `{module}.domain.*` | 禁止 | 模块私有领域模型 |
| `{module}.application.*` | 禁止 | 模块内部用例编排 |
| `{module}.internal.*` | 禁止 | 模块内部实现细节 |
| `{module}.protocol.*` | 禁止 | provider/tool 等外部协议 DTO |

治理目标：

```text
跨模块只能依赖 api/port/contract/common/bootstrap-core。
禁止因为测试方便或转换方便 import 对方 domain/internal。
storage 只能 import port，不 import api/domain/application/internal。
provider protocol DTO 只能在 provider internal/protocol 内部使用。
```
...
| `model.artifact` | 轻量 `ArtifactRefContract` 进入 `contract`；完整产物模型进入 `vi-agent-core-artifact` |
...
29. Checkpoint、Memory、Artifact、Audit 必须分离；它们可以互相引用 ID，但不能互相替代。
30. Graph checkpoint 必须通过 GraphCheckpointStore port 持久化，storage 提供实现。
31. ASK / Human Approval 必须产生 InterruptRequest、RuntimeResumeToken、ResumeCommand 和 audit 记录。
32. ToolRuntime 必须抽象 ToolTransport，MCP/HTTP/Sandbox 不能绕过 ToolPolicy/Safety/Audit。
33. 带副作用工具必须具备 idempotency key、timeout、cancellation 和 side-effect audit。
34. Streaming 对外事件必须统一为 RuntimeStreamEvent，不允许各模块直接输出私有事件格式。
35. Provider 必须提供 ModelProfile、routing、fallback、rate limit、budget 和 capability negotiation 边界。
36. RedactionPolicy 必须覆盖 audit、prompt、tool arguments、tool output、memory candidate 和 approval request。
37. Online validation 与 offline evaluation 分离，在线主链路不能依赖离线评测系统。
38. CapabilitySnapshot 的事实源是 AgentCapabilityResolver / CapabilityPackageAssembler 生成的执行快照，不属于 Memory。
...
Checkpoint / Interrupt / Resume / Replay 边界
Memory / Checkpoint / Artifact / Audit 边界分离
Tool MCP / Sandbox / Approval / Timeout / Cancellation / Side-effect Idempotency 边界
Streaming 统一事件模型
Model routing / fallback / rate limit / budget / capability negotiation 边界
Redaction 策略与 CapabilitySnapshot 事实源
...
验收命令
Codex 执行 prompt
完整 Graph Kernel / Checkpoint / Replay 实现细节
完整 Skill Runtime 实现细节
完整 Safety / Approval / Evaluation / RAG 字段契约与存储设计
完整 Artifact 存储实现和 Workspace 设计
独立 Docker 服务拆分方案
P3/P4/P5 详细计划
```
...
lifecycle / agent / graph / skill / tool / prompt / context / memory
  +
provider / storage / artifact / audit / safety / evaluation / rag
  +
app
```
...
Checkpoint / Interrupt / Resume 是 Graph 执行恢复边界，不属于 Memory。
Artifact 管理任务产物生命周期，Memory/Audit/Checkpoint 只引用 ArtifactRef。
Streaming 事件由 lifecycle 统一对外输出。
ToolRuntime 必须支持 ToolTransport、Sandbox、Timeout、Cancellation、Approval、Redaction 与 Side-effect Idempotency。
```
